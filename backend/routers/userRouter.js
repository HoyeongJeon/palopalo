...
// 원래 patch
userRouter.patch('/me/edit', authMiddleware, fileUpoadMiddleware.single('profile_picture'), async (req, res) => {
  const {
  ...
  user.favorite_weather = favorite_weather ? favorite_weather : meInfo.favorite_weather;
  ...
  try {
    ...
    console.error(error);
    return res.status(500).json({ ...resBody(false, '내 정보 수정에 실패했습니다.') });
  }
});
...
userRouter.patch('/me/password-edit', authMiddleware, async (req, res) => {
  ...
  // 비밀번호를 입력하지 않은 경우
  if (!password) {
    return res.status(400).json({ ...resBody(false, '비밀번호를 입력해주세요.') });
  }
  ...
  if (!isMatch) {
    return res.status(400).json({ ...resBody(false, '비밀번호가 일치하지 않습니다.') });
  }

  if (newPassword.length < 6 && newPasswordCheck.length < 6) {
    return res.status(400).json({ ...resBody(false, '비밀번호는 최소 6자리 이상이어야 합니다.') });
  }

  if (newPassword !== newPasswordCheck) {
    return res.status(400).json({ ...resBody(false, '비밀번호 확인이 일치하지 않습니다.') });
  }
  // 새로운 비밀번호 암호화
  const hashedNewPassword = await bcrypt.hash(newPassword, Number(process.env.SALT));
  ...
  try {
    ...
    return res.status(200).json({ ...resBody(true, '비밀번호가 수정되었습니다.') });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ ...resBody(false, '비밀번호 수정에 실패했습니다.') });
  }
});
