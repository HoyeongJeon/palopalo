...
authRouter.post('/signup', validationCheck, async (req, res) => {
  const { email, password, passwordCheck, name, nickname, location, introduce, favorite_weather } = req.body;
  if (!email || !password || !name || !passwordCheck || !location || !nickname || !introduce) {
  ...
  try {
    ...
    const userInfo = await Userinfo.create({
      ...
      location,
      introduce,
      favorite_weather: favorite_weather ? favorite_weather : '모든 날씨가 다 좋아~🎶',
...
authRouter.post('/login', async (req, res) => {
  ...
  if (!email || !password) {
    return res.status(400).json({ ...resBody(false, '아이디 / 비밀번호를 입력해주세요.') });
  }
  ...
  if (!isMatch) {
    return res.status(400).json({ ...resBody(false, '잘못된 비밀번호입니다.') });
  }
...
authRouter.post('/logout', (req, res) => {
  try {
    ...
    console.error(error);
    return res.status(500).json({ ...resBody(500, '로그아웃에 실패했습니다.') });
  }
});
